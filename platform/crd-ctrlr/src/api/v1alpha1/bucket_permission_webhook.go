...
// +kubebuilder:webhook:path=/validate-sdewan-bucket-permission,mutating=false,failurePolicy=fail,groups="batch.sdewan.akraino.org",resources=mwan3policies,verbs=create;update;delete,versions=v1alpha1,name=validate-sdewan-bucket
...
func (v *bucketPermissionValidator) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	bucketlog.Info(fmt.Sprintf("req.userinfo: %v", req.UserInfo))
	if err != nil {
		bucketlog.Error(err, "Failed to get rolebinding list")
	} else {
		bucketlog.Info(fmt.Sprintf("rolebindings: %v", roleBindings))
		for _, rolebinding := range roleBindings.Items {
			...
			if role.Namespace == meta.Namespace {
				resourcePer := perm[req.Resource.Resource]
				if resourcePer != nil {
	...
	if err != nil {
		...
		bucketlog.Info(fmt.Sprintf("clusterrolebindings: %v", clusterRoleBindings))
